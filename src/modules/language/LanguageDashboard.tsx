...
export function LanguageDashboard() {
  ...
  const clientsTable = useTable<ClientRecord>({
    ...
    supabaseFilter: { column: "department", value: "language" },
  });
  ...
  const handleExport = (format: "pdf" | "excel") => {
    ...
    if (activeTab === "Clients") {
      ...
      if (format === "pdf") exportToPDF({ title: "Language Clients Report", headers, rows, fileName: "language_clients" });
      else exportToExcel({ title: "Language Clients", headers, rows, fileName: "language_clients" });
      ...
      if (format === "pdf") exportToPDF({ title: "Language Tasks Report", headers, rows, fileName: "language_tasks" });
      else exportToExcel({ title: "Language Tasks", headers, rows, fileName: "language_tasks" });
      ...
      if (format === "pdf") exportToPDF({ title: "Language Goals Report", headers, rows, fileName: "language_goals" });
      else exportToExcel({ title: "Language Goals", headers, rows, fileName: "language_goals" });
  ...
  // Client CRUD
  const openAddClient = () => {
    setClientForm({ name: "", serviceType: "Document Translation", tasksCount: "0", paymentAmount: "PKR 0", paymentStatus: "Pending" });
  ...
  const openEditClient = (client: ClientRecord) => {
    setClientForm({ name: client.name, serviceType: client.serviceType, tasksCount: client.tasksCount, paymentAmount: client.paymentAmount, paymentStatus: client.paymentStatus });
  ...
  const openEditTask = (task: TaskRecord) => {
    setTaskForm({ title: task.title, assignee: task.assignee, priority: task.priority, dueDate: task.dueDate, progress: task.progress, status: task.status });
  ...
  const openEditGoal = (goal: GoalRecord) => {
    setGoalForm({ description: goal.description, target: goal.target, progress: goal.progress, pct: goal.pct, deadline: goal.deadline, status: goal.status });
  ...
  return (
    <>
      ...
      <section className="grid gap-6 2xl:grid-cols-[minmax(0,1.45fr)_minmax(380px,0.75fr)]">
        <div>
          {activeTab === "Clients" ? (
            <LanguageClientTableComponent table={clientsTable} openEdit={openEditClient} />
          ) : activeTab === "Tasks" ? (
            <LanguageTasksTableComponent table={tasksTable} openEdit={openEditTask} />
          ) : (
            <LanguageGoalsTableComponent table={goalsTable} openEdit={openEditGoal} />
          )}
        </div>
        <LanguageRightColumn projectStatusData={projectStatusData} totalProjects={totalClients} />
      </section>

      {/* CRUD MODALS */}

      {/* Client Add */}
      <Modal isOpen={clientsTable.isAddOpen} onClose={() => clientsTable.setIsAddOpen(false)} title="Add New Language Client">
        <form onSubmit={submitAddClient} className="space-y-4">
          <label className="block">
            ...
            <input type="text" required value={clientForm.name} onChange={(e) => setClientForm({ ...clientForm, name: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" placeholder="e.g. Ahmad Siddiqui" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Service Type</span>
            <select value={clientForm.serviceType} onChange={(e) => setClientForm({ ...clientForm, serviceType: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
          ...
          <label className="block">
            ...
            <input type="text" required value={clientForm.paymentAmount} onChange={(e) => setClientForm({ ...clientForm, paymentAmount: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" placeholder="e.g. PKR 50,000" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Payment Status</span>
            <select value={clientForm.paymentStatus} onChange={(e) => setClientForm({ ...clientForm, paymentStatus: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
          ...
          <div className="flex justify-end gap-3 pt-3">
            <button type="button" onClick={() => clientsTable.setIsAddOpen(false)} className="h-10 rounded-xl border border-slate-300 px-4 text-sm font-extrabold text-slate-600 hover:bg-slate-50 bg-white">Cancel</button>
            <button type="submit" className="h-10 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-soft hover:bg-blue-700">Save Client</button>
          </div>
        </form>
      </Modal>

      {/* Client Edit */}
      <Modal isOpen={clientsTable.isEditOpen} onClose={() => clientsTable.setIsEditOpen(false)} title="Edit Language Client">
        <form onSubmit={submitEditClient} className="space-y-4">
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Client Name</span>
            <input type="text" required value={clientForm.name} onChange={(e) => setClientForm({ ...clientForm, name: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Service Type</span>
            <select value={clientForm.serviceType} onChange={(e) => setClientForm({ ...clientForm, serviceType: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
          ...
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Payment Amount</span>
            <input type="text" required value={clientForm.paymentAmount} onChange={(e) => setClientForm({ ...clientForm, paymentAmount: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Payment Status</span>
            <select value={clientForm.paymentStatus} onChange={(e) => setClientForm({ ...clientForm, paymentStatus: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
          ...
          <div className="flex justify-end gap-3 pt-3">
            <button type="button" onClick={() => clientsTable.setIsEditOpen(false)} className="h-10 rounded-xl border border-slate-300 px-4 text-sm font-extrabold text-slate-600 hover:bg-slate-50 bg-white">Cancel</button>
            <button type="submit" className="h-10 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-soft hover:bg-blue-700">Save Changes</button>
          </div>
        </form>
      </Modal>
        ...
        {clientsTable.selectedItem ? (
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-card space-y-3">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Client Details</span>
              <p className="text-sm font-semibold text-slate-600">ID: <b className="text-slate-800">#{clientsTable.selectedItem.id}</b></p>
              <p className="text-sm font-semibold text-slate-600">Name: <b className="text-slate-800">{clientsTable.selectedItem.name}</b></p>
            </div>
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-card space-y-3">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Service Info</span>
              <p className="text-sm font-semibold text-slate-600">Service: <b className="text-slate-800">{clientsTable.selectedItem.serviceType}</b></p>
              <p className="text-sm font-semibold text-slate-600">Tasks: <b className="text-slate-800">{clientsTable.selectedItem.tasksCount}</b></p>
            </div>
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-card space-y-3">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Payment</span>
              <p className="text-sm font-semibold text-slate-600">Amount: <b className="text-slate-800">{clientsTable.selectedItem.paymentAmount}</b></p>
      ...
      {/* Client Delete */}
      <ConfirmDialog isOpen={clientsTable.isConfirmOpen} onClose={() => clientsTable.setIsConfirmOpen(false)} onConfirm={confirmDeleteClient} title="Delete Client" message={`Remove ${clientsTable.selectedItem?.name} from language service records?`} />

      {/* Task Add */}
      <Modal isOpen={tasksTable.isAddOpen} onClose={() => tasksTable.setIsAddOpen(false)} title="Create Language Task">
        <form onSubmit={submitAddTask} className="space-y-4">
          <label className="block">
            ...
            <input type="text" required value={taskForm.title} onChange={(e) => setTaskForm({ ...taskForm, title: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" placeholder="e.g. Translate Legal Documents" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Assigned To</span>
            <input type="text" required value={taskForm.assignee} onChange={(e) => setTaskForm({ ...taskForm, assignee: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" placeholder="e.g. Fatima Sheikh" />
          </label>
          <div className="flex gap-3">
            <label className="block flex-1">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Priority</span>
              <select value={taskForm.priority} onChange={(e) => setTaskForm({ ...taskForm, priority: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
                <option>High</option>
            ...
            <label className="block flex-1">
              ...
              <select value={taskForm.status} onChange={(e) => setTaskForm({ ...taskForm, status: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
          ...
          <div className="flex justify-end gap-3 pt-3">
            <button type="button" onClick={() => tasksTable.setIsAddOpen(false)} className="h-10 rounded-xl border border-slate-300 px-4 text-sm font-extrabold text-slate-600 hover:bg-slate-50 bg-white">Cancel</button>
            <button type="submit" className="h-10 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-soft hover:bg-blue-700">Create Task</button>
          </div>
        </form>
      </Modal>

      {/* Task Edit */}
      <Modal isOpen={tasksTable.isEditOpen} onClose={() => tasksTable.setIsEditOpen(false)} title="Edit Language Task">
        <form onSubmit={submitEditTask} className="space-y-4">
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Task Title</span>
            <input type="text" required value={taskForm.title} onChange={(e) => setTaskForm({ ...taskForm, title: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Assigned To</span>
            <input type="text" required value={taskForm.assignee} onChange={(e) => setTaskForm({ ...taskForm, assignee: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
          </label>
          <div className="flex gap-3">
            <label className="block flex-1">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Priority</span>
            ...
            <label className="block flex-grow">
              ...
              <input type="text" required value={taskForm.progress} onChange={(e) => setTaskForm({ ...taskForm, progress: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
            </label>
            <label className="block flex-1">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Status</span>
              <select value={taskForm.status} onChange={(e) => setTaskForm({ ...taskForm, status: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
          ...
          <div className="flex justify-end gap-3 pt-3">
            <button type="button" onClick={() => tasksTable.setIsEditOpen(false)} className="h-10 rounded-xl border border-slate-300 px-4 text-sm font-extrabold text-slate-600 hover:bg-slate-50 bg-white">Cancel</button>
            <button type="submit" className="h-10 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-soft hover:bg-blue-700">Save Changes</button>
          </div>
        </form>
      </Modal>
      ...
      {/* Goal Add */}
      <Modal isOpen={goalsTable.isAddOpen} onClose={() => goalsTable.setIsAddOpen(false)} title="Register Language Goal">
        <form onSubmit={submitAddGoal} className="space-y-4">
          <label className="block">
            ...
            <input type="text" required value={goalForm.description} onChange={(e) => setGoalForm({ ...goalForm, description: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" placeholder="e.g. Expand translation capacity by 30%" />
          </label>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Target Deadline</span>
            <input type="text" required value={goalForm.deadline} onChange={(e) => setGoalForm({ ...goalForm, deadline: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" placeholder="e.g. 30 Jun 2024" />
          </label>
          <div className="flex justify-end gap-3 pt-3">
            <button type="button" onClick={() => goalsTable.setIsAddOpen(false)} className="h-10 rounded-xl border border-slate-300 px-4 text-sm font-extrabold text-slate-600 hover:bg-slate-50 bg-white">Cancel</button>
            <button type="submit" className="h-10 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-soft hover:bg-blue-700">Save Goal</button>
          </div>
        </form>
      </Modal>

      {/* Goal Edit */}
      <Modal isOpen={goalsTable.isEditOpen} onClose={() => goalsTable.setIsEditOpen(false)} title="Edit Language Goal">
        <form onSubmit={submitEditGoal} className="space-y-4">
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Goal Description</span>
            <input type="text" required value={goalForm.description} onChange={(e) => setGoalForm({ ...goalForm, description: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
          </label>
          <div className="flex gap-3">
            <label className="block flex-grow">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Target</span>
              <input type="text" required value={goalForm.target} onChange={(e) => setGoalForm({ ...goalForm, target: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
            </label>
            <label className="block flex-grow">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Progress</span>
              <input type="text" required value={goalForm.progress} onChange={(e) => setGoalForm({ ...goalForm, progress: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
            </label>
          </div>
          <div className="flex gap-3">
            <label className="block flex-grow">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">% Achieved</span>
              <input type="text" required value={goalForm.pct} onChange={(e) => setGoalForm({ ...goalForm, pct: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
            </label>
            <label className="block flex-grow">
              <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Deadline</span>
              <input type="text" required value={goalForm.deadline} onChange={(e) => setGoalForm({ ...goalForm, deadline: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white" />
            </label>
          </div>
          <label className="block">
            <span className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Status</span>
            <select value={goalForm.status} onChange={(e) => setGoalForm({ ...goalForm, status: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold text-slate-700 outline-none focus:border-brand bg-white">
              <option>Not Started</option>
              <option>In Progress</option>
              <option>Completed</option>
              <option>At Risk</option>
          ...
          <div className="flex justify-end gap-3 pt-3">
            <button type="button" onClick={() => goalsTable.setIsEditOpen(false)} className="h-10 rounded-xl border border-slate-300 px-4 text-sm font-extrabold text-slate-600 hover:bg-slate-50 bg-white">Cancel</button>
            <button type="submit" className="h-10 rounded-xl bg-brand px-5 text-sm font-extrabold text-white shadow-soft hover:bg-blue-700">Save Changes</button>
          </div>
        </form>
      </Modal>
...
function LanguageClientTableComponent({ table, openEdit }: { table: ReturnType<typeof useTable<ClientRecord>>; openEdit: (c: ClientRecord) => void }) {
  const columns = ["#", "Client Name", "Type of Service", "Tasks", "Payment Amount", "Payment Status", "Actions"];
  return (
    <Card title="Client Management" subtext="The Language Services module manages translation projects, language training programs, and client deliverable tracking across multiple languages." action={<TableActions searchQuery={table.searchQuery} onSearchChange={table.setSearchQuery} searchPlaceholder="Search clients..." columns={columns} visibleColumns={table.visibleColumns} onToggleColumn={(col) => table.setVisibleColumns((c) => ({ ...c, [col]: c[col] === false }))} />}>
          ...
          <tbody className="divide-y divide-slate-100">
            {table.paginatedData.map((row, index) => (
              <tr key={row.id} className="text-sm hover:bg-slate-50 transition duration-100">
                ...
                {table.visibleColumns["Type of Service"] !== false && <td className="px-4 py-3 font-semibold text-slate-600">{row.serviceType}</td>}
...
function LanguageTasksTableComponent({ table, openEdit }: { table: ReturnType<typeof useTable<TaskRecord>>; openEdit: (t: TaskRecord) => void }) {
  const columns = ["#", "Task Title", "Assigned To", "Priority", "Due Date", "Progress", "Status", "Actions"];
  return (
    <Card title="Tasks for Growth" subtext="Track and manage language service tasks, project deadlines, and team assignments." action={<TableActions searchQuery={table.searchQuery} onSearchChange={table.setSearchQuery} searchPlaceholder="Search tasks..." columns={columns} visibleColumns={table.visibleColumns} onToggleColumn={(col) => table.setVisibleColumns((c) => ({ ...c, [col]: c[col] === false }))} />}>
      <div className="scrollbar-thin overflow-x-auto">
...
function LanguageGoalsTableComponent({ table, openEdit }: { table: ReturnType<typeof useTable<GoalRecord>>; openEdit: (g: GoalRecord) => void }) {
  const columns = ["#", "Goal Description", "Target", "Current Progress", "% Achieved", "Deadline", "Status", "Actions"];
  return (
    <Card title="Goals Status" subtext="Monitor language department goals, targets, and achievement milestones." action={<TableActions searchQuery={table.searchQuery} onSearchChange={table.setSearchQuery} searchPlaceholder="Search goals..." columns={columns} visibleColumns={table.visibleColumns} onToggleColumn={(col) => table.setVisibleColumns((c) => ({ ...c, [col]: c[col] === false }))} />}>
      <div className="scrollbar-thin overflow-x-auto">
        <table className="w-full min-w-[1050px] text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {table.paginatedData.map((row, index) => (
              <tr key={row.id} className="text-sm hover:bg-slate-50 transition duration-100">
                {table.visibleColumns["#"] !== false && <td className="px-4 py-3 font-semibold text-slate-700">{(table.currentPage - 1) * table.pageSize + index + 1}</td>}
...
function LanguageRightColumn({ projectStatusData, totalProjects }: { projectStatusData: { name: string; value: number; color: string }[]; totalProjects: number }) {
  return (
    <aside className="space-y-6">
            ...
            <>
              <div className="relative h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={projectStatusData} innerRadius={72} outerRadius={104} paddingAngle={3} dataKey="value">
      ...
      <Card title="Top Language Services by Revenue">
